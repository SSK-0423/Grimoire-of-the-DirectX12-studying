...
float4 ps(Output input) : SV_TARGET
{
    ...
    ret += tex.Sample(smp, input.uv + float2(-128 * dx, -2 * dy));    //左上
    ret += tex.Sample(smp, input.uv + float2(0, -128 * dy));          //上
    ret += tex.Sample(smp, input.uv + float2(128 * dx, -2 * dy));     //右上
    ...
    ret += tex.Sample(smp, input.uv + float2(-128 * dx, 2 * dy));     //左下
    ret += tex.Sample(smp, input.uv + float2(0, 128 * dy));           //下
    ret += tex.Sample(smp, input.uv + float2(128 * dx, 2 * dy));      //右下
